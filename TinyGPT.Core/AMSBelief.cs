	...
	public sealed class AdaBelief : IDisposable
	{
		...
		public void Step(double learningRate, bool final, bool fastAdaBelief, double random_variance)
		{
			...
			for (int i = 0, size = tensors.Length; i < size; ++i)
			{
				...
				{
					Tensor exp_avg = mystate.exp_avg.to(device);
					Tensor exp_avg_sq = mystate.exp_avg_sq.to(device);
					...
					using(exp_avg){
						exp_avg.copy_(mystate.exp_avg);
					}
					using (exp_avg_sq){
						exp_avg_sq.copy_(mystate.exp_avg_sq);
					}
				}
